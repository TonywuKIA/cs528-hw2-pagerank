...
BUILD_ID = os.environ.get("BUILD_ID", "hw8-vm")
...
def metadata_value(path: str) -> str:
...
def resolve_runtime_identity() -> tuple[str, str]:
    zone = ZONE
    if not zone:
        raw_zone = metadata_value("instance/zone")
        zone = raw_zone.rsplit("/", 1)[-1] if raw_zone else "unknown"

    instance_name = INSTANCE_NAME
    if not instance_name:
        instance_name = metadata_value("instance/name") or "unknown"

    return zone, instance_name


RUNTIME_ZONE, RUNTIME_INSTANCE_NAME = resolve_runtime_identity()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _write(self, body: str, status: int, content_type: str = "text/html; charset=utf-8") -> None:
        ...
        self.send_header("X-Zone", RUNTIME_ZONE)
        self.send_header("X-Instance-Name", RUNTIME_INSTANCE_NAME)
        self.end_headers()
    ...
    def _handle_get(self) -> None:
        ...
        if parsed.path == "/healthz":
            self._write(
                json.dumps({"status": "ok", "zone": RUNTIME_ZONE, "instance": RUNTIME_INSTANCE_NAME}),
                200,
        ...
        try:
            ...
            log_struct(severity="INFO", status=200, file=filename, msg="File served", zone=RUNTIME_ZONE)
            self._write(content, 200)
        except NotFound:
            log_struct(severity="WARNING", status=404, file=filename, msg="File not found", zone=RUNTIME_ZONE)
            self._write("Not found", 404)
        except Exception as exc:  # noqa: BLE001
            log_struct(severity="ERROR", status=500, msg="Internal error", error=str(exc), zone=RUNTIME_ZONE)
    ...
    def _method_not_implemented(self) -> None:
        log_struct(severity="WARNING", status=501, method=self.command, msg="Method not implemented", zone=RUNTIME_ZONE)
